...
const SidebarV6: React.FC<SidebarV6Props> = ({
  onLogout,
  ...
  return (
    <aside className={`sidebar sidebar--v6 ${isCollapsed ? "sidebar--collapsed" : ""}`} aria-label="Doc2Me Primary Navigation">
      {/* Brand & Context */}
      <div className="sidebar__brand-container">
        <div className="sidebar__brand" style={{ display: "flex", alignItems: "center", gap: "10px" }}>
          <span className="sidebar__brand-mark" aria-hidden="true" style={{ background: "linear-gradient(135deg, #FF7A00, #F43F5E)", color: "#fff", display: "inline-grid", placeItems: "center", width: "24px", height: "24px", borderRadius: "6px", fontSize: "14px", fontWeight: "bold" }}>D</span>
          {!isCollapsed && <span className="sidebar__brand-name" style={{ fontStyle: "normal", background: "linear-gradient(to right, #0F172A, #2563EB)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", fontWeight: 800, fontSize: "20px" }}>Doc2Me</span>}
        </div>
        {!isCollapsed && (
          <div className="sidebar__tagline" style={{ fontSize: "11px", fontWeight: 700, color: "var(--v6-blue)", marginTop: "8px", lineHeight: "1.4", textTransform: "uppercase", letterSpacing: "0.05em" }}>
            AI Health Platform
          </div>
        )}
      </div>
      ...
      <div className="sidebar__support" style={{ padding: "16px 12px", marginTop: "auto" }}>
        ...
        {!isCollapsed && (
          <div className="sidebar--v5-support-card" style={{ background: "linear-gradient(135deg, rgba(37, 99, 235, 0.05), rgba(124, 58, 237, 0.05))", border: "1px solid rgba(37, 99, 235, 0.15)", borderRadius: "8px", padding: "12px", marginTop: "12px" }}>
            <div className="title" style={{ fontSize: "11px", fontWeight: "bold", color: "#475569" }}>Need assistance?</div>
            <div className="email" style={{ fontSize: "12px", color: "var(--v6-blue)", fontWeight: "600", marginTop: "2px" }}>support@doc2me.com</div>
          </div>
        )}
      </div>
    </aside>
