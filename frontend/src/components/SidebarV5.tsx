...
const SidebarV5: React.FC<SidebarV5Props> = ({
  ...
  const filteredNavigationItems = navigationItems(userRole || "").filter(item => {
    // Only patients have V5 in scope, but we keep fallback logic robust
    if (userRole === "doctor") {
      return (
        item.tab === "dashboard" ||
        item.tab === "profile" ||
  ...
  return (
    <aside className={`sidebar sidebar--v5 ${isCollapsed ? "sidebar--collapsed" : ""}`} aria-label="Primary navigation">
      {/* Brand & Context */}
